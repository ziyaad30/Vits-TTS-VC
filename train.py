...
def run(rank, n_gpus, hps):
    ...
    if rank == 0:
        ...
        eval_loader = DataLoader(eval_dataset, num_workers=0, shuffle=False, batch_size=hps.train.batch_size, pin_memory=True, drop_last=False, collate_fn=collate_fn)
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, items in enumerate(pbar(train_loader)):
    x, x_lengths, spec, spec_lengths, y, y_lengths, speakers = items
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        pbar.write('Train Epoch: {} [{:.0f}%]'.format(epoch, 100. * batch_idx / len(train_loader)))
        # pbar.write(str([x.item() for x in losses] + [global_step, lr]))
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        if hps.preserved > 0:
          ...
          if os.path.exists(old_d):
            ...
            os.remove(old_d)
          if os.path.exists("/content/drive/MyDrive/"):
              utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch, os.path.join("/content/drive/MyDrive/", "G_{}.pth".format(global_step)))
              utils.save_checkpoint(net_d, None, hps.train.learning_rate, epoch, os.path.join("/content/drive/MyDrive/", "D_{}.pth".format(global_step)))
              old_g = utils.oldest_checkpoint_path("/content/drive/MyDrive/", "G_[0-9]*.pth", preserved=hps.preserved)  # Preserve 4 (default) historical checkpoints.
